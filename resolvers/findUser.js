  ...
  Query: {
    searchUsers: async (_, { username, limit, offset }, context) => {
      ...
      const enrichedResults = results.map((user) => {
        ...
        const currentUserLinks =
          currentUser.links?.map((id) => String(id)) || [];
        const currentUserSentLinks =
          currentUser.sentLinks?.map((id) => String(id)) || [];
        const currentUserReceivedLinks =
          currentUser.receivedLinks?.map((id) => String(id)) || [];
    ...
    getUser: async (_, { id }, context) => {
      try {
        ...
        const [totalLinks] = await Promise.all([user?.links?.length || 0]);
        ...
        if (privacySettings?.privacySettings?.visibleContact === false) {
          userToReturn.email = null;
          userToReturn.contactNumber = null;
        }
